...
function craeteCart(listofcartProd) {
  if (listofcartProd.length !== 0) {
    for (let i in listofcartProd) fetchui(listofcartProd[i]);
  }
}
...
function createUi(product, q) {
  ...
  imgcaualdiv.append(img);
  prodImagHolder.append(imgcaualdiv);
  ...
  promotion.textContent = "promotional details";
